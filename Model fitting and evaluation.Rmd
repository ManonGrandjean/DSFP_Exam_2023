...
accuracy(fit_volt) %>% select(.model, machineID, RMSE, MAPE) %>%
  aggregate(cbind(RMSE, MAPE) ~ .model, FUN = "mean", drop = FALSE)

accuracy(fit_rotate) %>% select(.model, machineID, RMSE, MAPE) %>%
  aggregate(cbind(RMSE, MAPE) ~ .model, FUN = "mean", drop = FALSE)

accuracy(fit_pressure) %>% select(.model, machineID, RMSE, MAPE) %>%
  aggregate(cbind(RMSE, MAPE) ~ .model, FUN = "mean", drop = FALSE)

accuracy(fit_vibration) %>% select(.model, machineID, RMSE, MAPE) %>%
  aggregate(cbind(RMSE, MAPE) ~ .model, FUN = "mean", drop = FALSE)

accuracy(fit_daysSinceError) %>% select(.model, machineID, RMSE, MAPE) %>%
  aggregate(cbind(RMSE, MAPE) ~ .model, FUN = "mean", drop = FALSE)
